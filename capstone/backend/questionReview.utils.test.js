...
test('active AI generation says remaining questions are automatic instead of asking for manual additions', () => { const result = buildQuestionCountEligibility({ file_type: 'lesson', requested_question_count: 25, generation_status: 'generating' }, 5, 'publication'); assert.equal(result.eligible, false); assert.match(result.message, /5 of 25/); assert.match(result.message, /remaining 20 questions are still being generated automatically/i); assert.doesNotMatch(result.message, /Add 20 questions/i); });
