...
const clearStudentLearningData = async (client, studentIds) => {
  const ids = Array.from(new Set((studentIds || []).map((value) => normalizeId(value, 'Student account ID'))));
  if (ids.length === 0) return;

  await client.query('DELETE FROM public.game_results WHERE resolved_student_id = ANY($1::INTEGER[])', [ids]);
  await client.query('DELETE FROM public.playtime_sessions WHERE student_id = ANY($1::INTEGER[])', [ids]);
  await client.query('DELETE FROM public.student_game_progress WHERE student_id = ANY($1::INTEGER[])', [ids]);
  await client.query('DELETE FROM public.student_quest_milestones WHERE student_id = ANY($1::INTEGER[])', [ids]);
  await client.query('DELETE FROM public.student_ai_insights WHERE student_id = ANY($1::INTEGER[])', [ids]);
};

const archiveParentFamily = async (pool, parentId) => withFamilyTransaction(pool, async (client) => {
  ...
  if (!parentResult.rows[0]) throw createFamilyError('Parent account could not be archived.', 409);

  return {
    archivedParent: parentResult.rows[0],
    archivedChildren: children,
    archivedStudentIds: studentIds,
  };
});

const restoreParentFamily = async (pool, parentId) => withFamilyTransaction(pool, async (client) => {
  ...
  const parentResult = await client.query(
    `UPDATE public.accounts
     SET is_archived = false,
         status = CASE WHEN LOWER(COALESCE(status, '')) = 'offline' THEN 'Offline' ELSE status END,
         session_version = COALESCE(session_version, 0) + 1
     WHERE id = $1
     RETURNING *`,
    [parent.id]
  );
  if (!parentResult.rows[0]) throw createFamilyError('Parent account could not be restored.', 409);
  ...
  return {
    restoredParent: parentResult.rows[0],
    restoredStudentIds: studentIds,
  };
});

const permanentlyDeleteLegacySixDigitStudents = async (pool) => withFamilyTransaction(pool, async (client) => {
  const result = await client.query(
